...
namespace Spectre
{
    namespace Server
    {
        ...
        int MaxClients = 1; // Maximum amount of clients.
        bool IsProtected = false; // Protect server from out-of-network connections.
        ...
        struct sockaddr_in server; // Server socket data.
        int activity; // Socket activity.
        int addrlen; // Address length.
        int valread; // Value reading data.
        int maxbuffer = 65536; // Maximum buffer length. (do not change for now)
        ...
        int Start(void action(std::string script))
        {
            ...
            while (!closed)
            {
                ...
                if (FD_ISSET(master_socket, &readfds))
                {
                    ...
                    if (inet_ntoa(address.sin_addr) != limiter && IsProtected == true)
                    {
                            ...
                            else
                            {
                                ...
                                else
                                {
                                    ...
                                    else // Buffer is finished, move on
                                    {
                                        ...
                                        if (strlen(buffer) == 4 && strstr(buffer, "exit")) // If requested value if to close the server, handle it, if not move on
                                        {
                                            Spectre::Server::Close(); // Close the server
                                        }
                                        else
                                        {
            ...
            printf("[Spectre -> Debug] Cleaning up..\n");
